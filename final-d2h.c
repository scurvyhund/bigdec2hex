...
// Helper function to perform division of a large number represented as a string by a small integer.
// It modifies the number string in-place to be the quotient and returns the remainder.
...
char* decimal_to_hex_string_bigint(const char* decimal_str) {
    ...
    // Allocate a buffer for the hex string. A safe upper bound is the number of decimal digits.
...
// The format_hex_with_padding function remains the same as the last correct version.
char* format_hex_with_padding(const char* input_hex) {
    if (input_hex == NULL) return NULL;
    size_t input_len = strlen(input_hex);
    if (input_len == 0) { char* empty_str = (char*)malloc(8); if(empty_str) strcpy(empty_str, "Hex: ()"); return empty_str; }
    ...
    if (formatted_str == NULL) { fprintf(stderr, "Error: Memory allocation failed.\n"); return NULL; }
